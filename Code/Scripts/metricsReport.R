...
getDrivers <- function(rma) {
  ...
  groupedDrivers2 <- summarise(groupedDrivers1, "# of RMAs" = length(unique(RMA_ID)),  Qty = sum(Return_Qty), SP_Total_Price = sum(SP_Total_Price))
  groupedDrivers2 <- groupedDrivers2[,c(5, 6, 1, 2, 3, 4, 7)]
